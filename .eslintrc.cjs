module.exports = {
  ...
  rules: {
    ...
    'no-restricted-syntax': [
      ...
      {
        selector:
          'VariableDeclarator[init.callee.name=useFormState][id.type=Identifier]',
        message:
      ...
      {
        selector:
          'CallExpression[callee.type=MemberExpression][callee.property.name=forEach]:has(AwaitExpression)',
        message:
      ...
      {
        selector:
          'JSXElement[openingElement.name.property.name=Provider] JSXElement[openingElement.name.name]',
        message:
      ...
      },
      {
        ...
        message:
          'Do not use JSON.parse(JSON.stringify(...)) for deep copy. Use structuredClone instead.',
      },
      {
        selector:
          'CallExpression[callee.name=test][arguments.0.value!=/^should/]',
